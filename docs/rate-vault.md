...
First-class workshop modules (`RATE_VAULT_SECTIONS` ids): `halls`, `contractor`, `cba-pla`, `p66`, `publish`.
...
1. Keep `lib/rate-vault.ts` as the public data/API contract (workshop shape, section ids including `cba-pla`, empty CBA/PLA vault, publish stub).
...
4. When live hall / contractor / CBA-PLA / P66 ingest lands, persist it behind this module — not `lib/estimate-pack.ts` or `/api/desk/rates`. `cba-pla` stays a first-class module id, not a child of `p66`.
