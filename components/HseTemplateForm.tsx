...
export function HseTemplateForm({
  ...
  const viewOnly = hseTemplateFormViewOnly(fillAcl, lens);
  const canSave = !viewOnly && hseTemplateCanSave(fillAcl, dest);
            ...
            {formDef.fields.map((field) => (
              <FieldBlock key={field.id} label={field.label}>
                <DeskTemplateFieldInput
                  ...
                  viewOnly={viewOnly}
                  onChange={(next) => setRecord(patchHseTemplateField(record, field.id, next))}
                />
              </FieldBlock>
            ))}
          </div>
