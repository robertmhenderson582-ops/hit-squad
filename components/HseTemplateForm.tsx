...
import { hseCompanyDocFileName } from "@/lib/hse-company-docs";
import {
  HSE_JOB_SCOPE_KEY,
  cascadeClients,
  cascadeJobs,
...
import {
  ...
  hseTemplateFormTitle,
  hseTemplateFormToLead,
  isMadisonJsaForm,
  removeHseTemplateRow,
  serializeHseTemplateForm,
  type HseTemplateFillAcl,
  type HseTemplateFormPayload,
  type HseTemplateFillDest,
  type HseTemplateFormDef,
  type HseTemplateFormRecord,
  type HseTemplateSourceKind,
} from "@/lib/hse-template-form";
import { HSE_VAULT_WRITE_ERROR } from "@/lib/hse-vault-shared";
import type { QualityFieldDef } from "@/lib/quality-day1";

export type HseTemplateFormSession = {
...
export function HseTemplateForm({
  open,
  session,
  clients,
  sites,
  ...
  open: boolean;
  session: HseTemplateFormSession | null;
  ...
  const def = session
    ...
    : null;
  const [record, setRecord] = useState<HseTemplateFormRecord>(() =>
    def ? emptyHseTemplateFormRecord(def) : { fields: {}, rows: [] },
  );
  const [dest, setDest] = useState<HseTemplateFillDest>(session?.dest || (jobPick.jobId ? "job" : "prepackage"));
  ...
  const title = def ? hseTemplateFormTitle(def, session?.fileName) : "HSE form";
  ...
  const readOnly = fillAcl.readOnly || !hseTemplateCanSave(fillAcl, dest);
  const canSave = hseTemplateCanSave(fillAcl, dest);
  ...
  async function persist() {
    if (!canSave) {
      setNote(HSE_TEMPLATE_FILL_VIEW_ERROR);
      return;
    }
    if (!hseTemplateFormHasWork(record)) {
      setNote(HSE_TEMPLATE_FILL_EMPTY_ERROR);
      return;
    }
    ...
    const payload: HseTemplateFormPayload = {
    ...
    const file = hseTemplateFormToLead(payload, name);
    try {
  ...
  function downloadFilled() {
    const text = serializeHseTemplateForm({
      mark: HSE_TEMPLATE_FORM_MARK,
      id: formDef.id,
      title: formDef.title,
      folderId: formDef.folderId,
  ...
  return (
    <ModalPortal>
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="hse-template-form-title"
        id="hse-template-form"
        onClick={onClose}
      >
        <div
        ...
        >
          <div className="flex items-start justify-between gap-3">
            <h2 id="hse-template-form-title" className="font-display text-2xl text-[#163038]">
              {title}
            </h2>
          ...
          {active.fileName && !isHseFilledCopyName(active.fileName) ? (
            <p className="mt-2 text-xs text-[#5b6f73]">
              Template on the rail: {hseCompanyDocFileName(active.fileName)}
            </p>
          ) : null}
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            {formDef.fields.map((field) => (
              <FieldBlock key={field.id} label={field.label}>
                {fieldInput(field, record.fields[field.id] || "", (next) => setRecord(patchHseTemplateField(record, field.id, next)), readOnly)}
              </FieldBlock>
            ))}
          </div>
          {formDef.rowFields.length ? (
            <FormRows
              ...
              rows={record.rows}
              readOnly={readOnly}
              onAdd={() => setRecord(addHseTemplateRow(record))}
              onPatch={(rowId, field, next) => setRecord(patchHseTemplateRow(record, rowId, field, next))}
              onRemove={(rowId) => setRecord(removeHseTemplateRow(record, rowId))}
            />
          ) : formDef.hint ? (
          ...
          <div className="mt-4 flex flex-wrap justify-end gap-3">
            {isMadisonJsaForm(formDef) ? (
              <button type="button" className="text-sm text-steel underline" onClick={() => window.print()}>
                Print Madison JSA
              </button>
            ) : null}
            {hseTemplateFormHasWork(record) ? (
            ...
            {canSave ? (
              <button
                ...
                disabled={saving || loading || !hseTemplateFormHasWork(record)}
...
export function readHseJobScopePick() {
  return readJobScope(HSE_JOB_SCOPE_KEY);
}
