...
import { HSE_DROP_ACCEPT, checkHseDrop } from "@/lib/hse-folders";
import { isHseFilledCopyName } from "@/lib/hse-template-form";
import type { HsePackageShelfAcl } from "@/lib/hse-package-shelf";
import {
...
export function HsePackageShelf({
  companyId,
  companyLabel,
  jobId,
  siteLabel,
  ...
  async function load() {
    ...
    const response = await fetch(
      `/api/desk/briefs?kind=hse&scope=package-shelf${company}`,
  ...
  async function persistKit(packageId: string, kitName: string, files: LeadFile[]) {
    ...
    if (!response.ok || (files.length > 0 && !hseVaultStored(data.store, data.stored))) {
  ...
  async function onFiles(list: FileList | File[] | null) {
    ...
    if (!kit) {
      setNoteKind("err");
      setNote("Create a Ready HSE package first.");
      return;
    }
    ...
    try {
      ...
      setNote(
        incoming.length === 1
          ? `Saved ${incoming[0].name} on the Ready HSE shelf.`
          : `Saved ${incoming.length} files on the Ready HSE shelf.`,
      );
      await load();
    } catch (error) {
      setNoteKind("err");
  ...
  return (
    <section className="plant-card px-4 py-4" aria-label="Ready HSE packages">
      <h2 className="font-display text-xl">Ready HSE packages</h2>
      <p className="mt-2 text-sm">
        {viewOnly
          ? "View only. Open a filled copy to read it. This seat cannot add, attach, replace, or remove Ready packages."
          : "Pre-build a job kit here before the job exists. It stays on the HSE vault shelf — not the company rail. HSE seats and a PM / estimator can attach it to an estimate or job."}
      </p>
      {acl.canBuild ? (
      ...
      {kits.length ? (
          ...
          {kits.map((kit) => {
            ...
            return (
              <li key={kit.id} className={`rounded-sm border px-3 py-2 ${selected ? "border-steel bg-steel/10" : "border-steel"}`}>
                ...
                {onOpenFilled && kit.files.some((file) => isHseFilledCopyName(file.name)) ? (
                  <ul className="mt-2 space-y-1">
                    {kit.files.filter((file) => isHseFilledCopyName(file.name)).map((file) => (
                      <li key={file.name}>
                        <button
                          type="button"
                          className="text-xs text-steel underline"
        ...
        <p className="mt-3 text-sm text-[#5b6f73]">No Ready HSE packages on the shelf yet.</p>
      )}
      ...
      {saving ? <p className="mt-2 text-sm">Saving to the HSE vault…</p> : null}
      {note ? (
