...
import { RODEO_U250_PACK_ID, rodeoU250FilledSnapshot } from "@/lib/madison-u250";
...
export function NewEstimateModal({
  ...
  async function createFromWorkbook(file: File) {
    ...
    try {
      ...
      if (classified.kind === "madison-contractor" && classified.packId === RODEO_U250_PACK_ID) {
        const { ingestMadisonU250 } = await import("@/lib/madison-u250-xlsx");
        const ingested = await ingestMadisonU250(bytes, file.name);
        const pack = rodeoU250FilledSnapshot({ ownerEmail: lens?.email || "" }, ingested);
        applyPackToStore(window.localStorage, pack);
        scheduleVaultUpsert(RODEO_U250_PACK_ID);
        onClose();
        router.push(`/estimates/${RODEO_U250_PACK_ID}`);
        return;
      }
    ...
    >
      ...
      >
        ...
        <div className="mt-3">
          ...
          <p className="estimate-ot-readout hud-readout mt-1 px-3 py-2 text-sm">{size === "shop" ? "Shop sheet" : alias(rule)}</p>
          <p className="mt-1 text-xs text-[#5b6f73]">
            {size === "shop"
