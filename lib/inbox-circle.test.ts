...
describe("inbox circle", () => {
  it("is the seven plus the Novus desk contact", () => {
    assert.deepEqual(
    ...
    assert.equal(INBOX_CIRCLE.length, 8);
    ...
    assert.equal(INBOX_CIRCLE.find((row) => row.id === "tester-joseph")?.company, "Hit Squad");
    ...
    assert.equal(canUseInbox({ email: JOSEPH_EMAIL }), true);
    assert.equal(canUseSuggestionBox({ email: JOSEPH_EMAIL }), true);
    assert.equal(canReceiveDeskBot({ email: JOSEPH_EMAIL }), true);
    assert.equal(isInboxCircleEmail(JOHN_BEECH_EMAIL), false);
    assert.equal(canUseInbox({ email: JOHN_BEECH_EMAIL }), false);
    ...
    assert.equal(
      inboxContactsFor(JOSEPH_EMAIL).some((row) => row.email === "robertmhenderson582@gmail.com"),
      true,
    );
    assert.equal(
      inboxContactsFor("robertmhenderson582@gmail.com").some((row) => row.email === JOSEPH_EMAIL),
      true,
    );
    assert.equal(inboxContactsFor(JOHN_BEECH_EMAIL).length, 0);
    ...
    assert.match(panel, /INBOX_CIRCLE_COPY/);
    ...
    assert.equal(INBOX_CIRCLE_COPY, "Inbox is the seven plus Novus.");
    assert.doesNotMatch(panel, /those six only/);
