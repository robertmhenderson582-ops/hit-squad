...
describe("Rate Vault scaffold", () => {
  ...
  it("wires Home door, route, and API without leaking into Inbox or Rate Tables", () => {
    ...
    assert.match(previewUi, /Tax BW/);
    assert.match(previewUi, /Hours Worked/);
    assert.match(previewUi, /Hours Paid/);
    assert.match(previewUi, /Straight Time/);
    assert.doesNotMatch(previewUi, /\bAccrual\b|hours-worked, hours-paid, straight-only/);
    ...
    assert.match(docs, /Tax BW/);
    assert.match(docs, /ST Calc/);
    assert.match(docs, /Hours Worked/);
    assert.match(docs, /Hours Paid/);
    ...
    assert.match(source("./rate-vault/b1-dropdowns.json"), /Hours Worked/);
    assert.match(source("./rate-vault/b1-dropdowns.json"), /Tax BW/);
