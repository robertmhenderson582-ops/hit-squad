...
describe("home doors", () => {
  it("keeps HIT SQUAD / PROJECT CONTROLS and buries Standalone off home", () => {
    ...
    assert.equal(HOME_DOCK_TILES.some((tile) => tile.key === "dispatch" && tile.href === "/onboard"), true);
    assert.equal(HOME_DOCK_TILES.find((tile) => tile.key === "dispatch")?.label, "Dispatch");
    ...
    assert.match(wordmark, /<BrandMark /);
    assert.equal(/variant="stacked"|variant="jets"/.test(wordmark), false);
    const mark = readFileSync(fileURLToPath(new URL("../components/BrandMark.tsx", import.meta.url)), "utf8");
    const favicon = readFileSync(fileURLToPath(new URL("../app/icon.svg", import.meta.url)), "utf8");
    assert.equal(/variant="stacked"|variant="jets"/.test(hero), false);
    assert.equal(/variant="stacked"|variant="jets"/.test(chrome), false);
    assert.match(hero, /<BrandMark /);
    assert.match(chrome, /<BrandMark /);
    assert.match(chrome, /home-title-mark/);
    assert.match(mark, /data-brand-mark="hud-a"/);
    assert.match(mark, /viewBox="0 0 80 80"/);
    assert.match(mark, /#3ec6d4/);
    assert.match(mark, /#0f5f6d/);
    assert.match(mark, /#e38b2a/);
    assert.match(mark, /M7 19\.5V7h12\.5/);
    assert.match(mark, /M68\.4 40 62\.2 36\.7v6\.6Z/);
    assert.equal(/jets|JET|translate\(48 4\)|translate\(4 34\)|viewBox="0 0 100 72"/.test(mark), false);
    assert.equal(/Madison|madison/.test(mark), false);
    assert.equal(/scale\(/.test(mark), false);
    assert.match(favicon, /#0f5f6d/);
    assert.match(favicon, /#e38b2a/);
    assert.match(favicon, /M7 19\.5V7h12\.5/);
    assert.equal(/rotate\(45 40 40\)/.test(favicon), false);
