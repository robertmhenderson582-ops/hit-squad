...
describe("Sample C home dock", () => {
  ...
  it("strips module links from the header and paints a clustered glass dock under the title", () => {
    ...
    assert.match(css, /\.home-dock \{\n  position: relative;/);
    assert.match(css, /\.home-dock \{\n  position: relative;[\s\S]{0,120}width: min\(50rem/);
    assert.match(css, /\.desk-home-root \.desk-hero \{[\s\S]{0,220}gap: 0\.95rem;/);
    ...
    assert.match(css, /\.home-dock-tile \{[\s\S]{0,220}min-width: 11\.75rem;/);
    assert.match(css, /\.home-dock-tile \{[\s\S]{0,280}min-height: 6\.15rem;/);
    assert.match(css, /\.home-dock-tile \{[\s\S]{0,280}flex: 0 0 11\.75rem;/);
    ...
    assert.match(css, /\.home-dock-label \{[\s\S]{0,240}font-size: 1\.32rem;/);
