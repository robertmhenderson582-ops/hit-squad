...
describe("Sample C home dock", () => {
  ...
  it("strips module links from the header and paints a clustered glass dock under the title", () => {
    ...
    assert.match(css, /\.home-dock \{\n  position: relative;/);
    assert.match(css, /\.home-dock \{\n  position: relative;[\s\S]{0,120}width: min\(46\.5rem/);
    assert.match(css, /\.desk-home-root \.desk-hero \{[\s\S]{0,220}gap: 1\.7rem;/);
    ...
    assert.match(css, /\.home-dock-tile \{[\s\S]{0,220}min-width: 10\.2rem;/);
    assert.match(css, /\.home-dock-tile \{[\s\S]{0,280}min-height: 5\.25rem;/);
    assert.match(css, /\.home-dock-tile \{[\s\S]{0,280}flex: 0 1 11rem;/);
    ...
    assert.match(css, /\.home-dock-label \{[\s\S]{0,240}font-size: 1\.22rem;/);
