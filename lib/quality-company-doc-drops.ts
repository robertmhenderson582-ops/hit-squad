...
export async function saveQualityCompanyDocDrop(user: QualityDocUser, input: QualityCompanyDocSaveInput) {
  ...
  try {
    ...
    console.warn(`quality-vault: company-doc write failed; ${status || "err"} ${driveFailureKind(error)}`);
    return {
      ok: false as const,
      status: 503,
...
export async function removeQualityCompanyDocFile(
  ...
  try {
    ...
    const status = error instanceof DriveApiError ? error.status : 0;
    console.warn(`quality-vault: company-doc delete failed; ${status || "err"} ${driveFailureKind(error)}`);
    return {
      ok: false as const,
      status: 503,
      error: qualityVaultWriteUserError(error, hasBuildDesk(user)),
...
export async function lockQualityCompanyDoc(
  ...
  try {
    ...
    const status = error instanceof DriveApiError ? error.status : 0;
    console.warn(`quality-vault: company-doc lock failed; ${status || "err"} ${driveFailureKind(error)}`);
    return {
      ok: false as const,
      status: 503,
      error: qualityVaultWriteUserError(error, hasBuildDesk(user)),
