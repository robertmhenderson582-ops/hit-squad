...
describe("inbox demo wipe", () => {
  ...
  it("stored demo threads are stripped and the cleaned list is persisted", () => {
    ...
    assert.equal(stored[0].id, "th-desk-v1.39");
  });
  ...
  it("stripDemoThreads drops the three demo ids", () => {
    ...
    assert.equal(cleaned[0].id, "th-desk-v1.39");
  ...
  it("Desk note stays V1.39 and tester-safe", () => {
    ...
    assert.equal(tester[0].id, "th-desk-v1.39");
    const testerBody = tester[0].messages.map((message) => message.text).join(" ");
    assert.match(testerBody, /V1\.39/);
    assert.match(testerBody, /Wage lookup is a tab on the estimate/);
    assert.match(testerBody, /Pick a position to see the wage for that job's site/);
    assert.match(testerBody, /old Rate books lookup button on Rates is gone/);
    ...
    assert.equal(owner[0].id, "th-desk-v1.39");
    const ownerBody = owner[0].messages.map((message) => message.text).join(" ");
    assert.match(ownerBody, /View as Nathan/);
    assert.match(ownerBody, /Madison button/);
    assert.match(ownerBody, /Shahan TM OCIP/);
  });
});
