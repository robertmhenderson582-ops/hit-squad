...
describe("inbox demo wipe", () => {
  ...
  it("stored demo threads are stripped and the cleaned list is persisted", () => {
    ...
    assert.equal(stored[0].id, "th-desk-v1.36");
  });
  ...
  it("stripDemoThreads drops the three demo ids", () => {
    ...
    assert.equal(cleaned[0].id, "th-desk-v1.36");
  ...
  it("Desk note stays V1.36 and tester-safe", () => {
    ...
    assert.equal(tester[0].id, "th-desk-v1.36");
    const testerBody = tester[0].messages.map((message) => message.text).join(" ");
    assert.match(testerBody, /V1\.36/);
    assert.match(testerBody, /Hours\/shift/);
    assert.match(testerBody, /Wood River third-party table/);
    ...
    assert.equal(owner[0].id, "th-desk-v1.36");
    const ownerBody = owner[0].messages.map((message) => message.text).join(" ");
    assert.match(ownerBody, /Staff Saturday is unchanged/);
    assert.match(ownerBody, /GF is not on Support Billed as/);
    assert.match(ownerBody, /markup % is unchanged/);
