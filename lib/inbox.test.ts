...
describe("inbox demo wipe", () => {
  ...
  it("stored demo threads are stripped and the cleaned list is persisted", () => {
    ...
    assert.equal(stored[0].id, "th-desk-v1.42");
  });
  ...
  it("stripDemoThreads drops the three demo ids", () => {
    ...
    assert.equal(cleaned[0].id, "th-desk-v1.42");
  ...
  it("Desk note stays V1.42 and tester-safe", () => {
    ...
    assert.equal(tester[0].id, "th-desk-v1.42");
    const testerBody = tester[0].messages.map((message) => message.text).join(" ");
    assert.match(testerBody, /V1\.42/);
    assert.match(testerBody, /Sign-in stays on this desk after a restart/);
    ...
    assert.equal(owner[0].id, "th-desk-v1.42");
    const ownerBody = owner[0].messages.map((message) => message.text).join(" ");
    assert.match(ownerBody, /Sign-in stays on this desk after a restart/);
    assert.match(ownerBody, /Owner sign-in is kept with the desk/);
  });
});
