...
describe("inbox demo wipe", () => {
  ...
  it("stored demo threads are stripped and the cleaned list is persisted", () => {
    ...
    assert.equal(stored[0].id, "th-desk-v1.48");
  });
  ...
  it("stripDemoThreads drops the three demo ids", () => {
    ...
    assert.equal(cleaned[0].id, "th-desk-v1.48");
  ...
  it("Desk note stays V1.48 and tester-safe", () => {
    ...
    assert.equal(tester[0].id, "th-desk-v1.48");
    const testerBody = tester[0].messages.map((message) => message.text).join(" ");
    assert.match(testerBody, /V1\.48/);
    assert.match(testerBody, /night counterparts/);
    assert.match(testerBody, /Rodeo form tab/);
    ...
    assert.equal(owner[0].id, "th-desk-v1.48");
    const ownerBody = owner[0].messages.map((message) => message.text).join(" ");
    assert.match(ownerBody, /night counterparts/);
