...
describe("inbox demo wipe", () => {
  ...
  it("stored demo threads are stripped and the cleaned list is persisted", () => {
    ...
    assert.equal(stored[0].id, "th-desk-v1.51.1");
  });
  ...
  it("stripDemoThreads drops the three demo ids", () => {
    ...
    assert.equal(cleaned[0].id, "th-desk-v1.51.1");
  ...
  it("Desk note stays V1.51.1 and tester-safe", () => {
    ...
    assert.equal(tester[0].id, "th-desk-v1.51.1");
    const testerBody = tester[0].messages.map((message) => message.text).join(" ");
    assert.match(testerBody, /V1\.51\.1/);
    ...
    assert.equal(owner[0].id, "th-desk-v1.51.1");
