...
describe("inbox demo wipe", () => {
  ...
  it("stored demo threads are stripped and the cleaned list is persisted", () => {
    ...
    assert.equal(stored[0].id, "th-desk-v1.51");
  });
  ...
  it("stripDemoThreads drops the three demo ids", () => {
    ...
    assert.equal(cleaned[0].id, "th-desk-v1.51");
  ...
  it("Desk note stays V1.51 and tester-safe", () => {
    assert.equal(DESK_VERSION, "1.51.0");
    const tester = applyWhatsNew([deskWhatsNewThread(false)], "tester-note", false, "nathanboyte@gmail.com");
    assert.equal(tester.length, 1);
    assert.equal(tester[0].id, "th-desk-v1.51");
    const testerBody = tester[0].messages.map((message) => message.text).join(" ");
    assert.match(testerBody, /V1\.51/);
    assert.match(testerBody, /Quality and HSE are readable field modules with fillable forms/);
    assert.match(testerBody, /Rolling chart: set up the bank/);
    ...
    assert.equal(owner[0].id, "th-desk-v1.51");
    const ownerBody = owner[0].messages.map((message) => message.text).join(" ");
    assert.match(ownerBody, /Quality and HSE are readable field modules with fillable forms/);
