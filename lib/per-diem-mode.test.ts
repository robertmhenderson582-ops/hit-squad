...
describe("per diem mode ripple", () => {
  ...
  it("Job setup + Other Cost + crew cards read the live pack switch — no parallel PD book", () => {
    ...
    const chrome = read("../app/globals.css");
    const analytics = read("../components/EstimateAnalyticsDesk.tsx");
    ...
    assert.match(chrome, /\.industrial-root \.choice-tile-on \{[\s\S]*background: rgba\(10, 40, 46, 0\.96\);/);
    assert.match(chrome, /\.industrial-root \.choice-tile-on \.choice-tile-title[\s\S]*color: #efe6d4;/);
    assert.match(chrome, /\.choice-tile-on \{[\s\S]*background: #fff;[\s\S]*color: #163038;/);
