...
test("leftover generation writes v2 and keeps Nathan Follow snapshot plus unrelated keys", () => {
  const keep = "hs_whats_new:1.51.1:owner";
  const store = memoryStore({
    [LENS_PACKS_LEGACY_KEY]: JSON.stringify({
  ...
  assert.ok(store.getItem(OWNER_PACKS_KEY));
  assert.ok(store.getItem(LENS_PACKS_KEY));
