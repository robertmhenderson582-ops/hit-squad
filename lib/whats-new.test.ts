...
describe("inbox what's-new", () => {
  it("seeds a per-seat Desk thread and keeps tester copy clean", () => {
    ...
    assert.match(TESTER_WHATS_NEW, /labeled/);
    assert.match(TESTER_WHATS_NEW, /Hours do not change/);
    ...
    assert.match(OWNER_WHATS_NEW, /Description/);
    assert.match(OWNER_WHATS_NEW, /clamp/);
