...
describe("inbox what's-new", () => {
  it("seeds a per-seat Desk thread and keeps tester copy clean", () => {
    ...
    assert.match(OWNER_WHATS_NEW, /Sites\/Plants card/);
    assert.match(TESTER_WHATS_NEW, /Capture grabs the whole desk, not one card/);
    assert.equal(TESTER_WHATS_NEW.includes("\n• "), true);
    assert.equal(TESTER_WHATS_NEW.split("\n").length, 2);
