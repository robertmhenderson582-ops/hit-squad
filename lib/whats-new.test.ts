...
describe("inbox what's-new", () => {
  it("seeds a per-seat Desk thread and keeps tester copy clean", () => {
    ...
    assert.equal(DESK_VERSION_LABEL, "Hit Squad Project Controls V1.34");
    assert.equal(DESK_THREAD_ID, "th-desk-v1.34");
  ...
  it("appends V1.34 onto an existing Hit Squad desk thread after V1.33", () => {
    assert.equal(seenKey("tester-x", "1.33.0"), `${WHATS_NEW_MARK_PREFIX}1.33.0:tester-x`);
    assert.equal(seenKey("tester-x"), `${WHATS_NEW_MARK_PREFIX}1.34.0:tester-x`);
    assert.notEqual(seenKey("tester-x", "1.33.0"), seenKey("tester-x"));
        ...
        messages: [
          {
            ...
            from: "them" as const,
            author: "Desk",
            text: "Hit Squad Project Controls V1.33",
            photo: null,
            sentAt: "",
            readAt: "seen",
          },
  ...
  it("posts the V1.34 Inbox note for testers and owner", () => {
    assert.equal(DESK_VERSION, "1.34.0");
    ...
    const live = applyWhatsNew([], "tester-v134-live", false, "nathanboyte@gmail.com");
    assert.equal(live[0].id, "th-desk-v1.34");
