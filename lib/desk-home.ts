import { companyDeskLogoSrc, isCompanyModuleKey, type Company } from "./companies.ts";
import { canSeeRateVaultDoor } from "./desk-role.ts";
import { canSeeCompanyModule } from "./module-access.ts";
...
function dockModuleVisible(
  key: string,
  session?: DockViewer | null,
  lens?: DockViewer | null,
  company?: Pick<Company, "modules"> | null,
) {
  if (key === ONBOARD_DOOR.key) {
    return canSeeOnboardDoor(session, lens) && canSeeCompanyModule(lens ?? session, company, "dispatch");
  }
  if (isCompanyModuleKey(key)) {
    return canSeeCompanyModule(lens ?? session, company, key);
  }
  return true;
}
 ...
 *  Company module-off hides that tile for the company's seats (Madison defaults all on). */
export function homeDockTilesForViewer(
  ...
  const dispatchOk = dockModuleVisible(ONBOARD_DOOR.key, session, lens, company);
  ...
  const tiles: HomeDockTile[] = homeDockTiles(canRates).filter((tile) =>
    dockModuleVisible(tile.key, session, lens, company),
  );
