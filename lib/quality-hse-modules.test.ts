...
describe("Quality and HSE have no estimate interaction", () => {
  ...
  it("fails if Chance’s Quality studio or named forms are missing", () => {
    ...
    const companyDocs = source("./quality-company-docs.ts");
    assert.match(companyDocs, /Quality Control Manual/);
    ...
    assert.match(folders, /NDE Request/);
    assert.match(folders, /WPS/);
    assert.match(folders, /Weld Log/);
    assert.match(folders, /Flange Log/);
    assert.match(folders, /Job Completion/);
    ...
    assert.doesNotMatch(quality, /bg-\[#fbf8f0\]/);
    assert.doesNotMatch(quality, /scrollIntoView/);
    const forms = source("../components/QualityFormScreens.tsx");
    assert.doesNotMatch(forms, /bg-\[#fbf8f0\]/);
    assert.match(forms, /role="tablist"/);
